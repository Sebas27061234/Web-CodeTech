...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var calendarAddEvent = function () {
    ...
    var mmm = String(currentDate.getMinutes()).padStart(2, "0");
    var ss = String(currentDate.getSeconds()).padStart(2, "0");
    var combineDate = `${yyyy}-${mm}-${dd}T${hh}:${mmm}:${ss}`;
    getModalAddBtnEl.style.display = "block";
    getModalUpdateBtnEl.style.display = "none";
    myModal.show();
    getModalStartDateEl.value = combineDate;
    getModalEndDateEl.value = 'yyyy-mm-ddT00:00:00';
  };
  ...
  var calendarEventClick = function (info) {
    ...
    if (eventObj.url) {
      ...
      getModalStartDateEl.value = eventObj.startStr.slice(0, 19);
      getModalEndDateEl.value = eventObj.endStr.slice(0, 19);
  ...
  function capitalizeFirstWord(text) {
    ...
    return `${words[0][0].toUpperCase()}${words[0].slice(1).toLowerCase()} ${words.slice(1).join(" ")}`;
  }

  var calendar1 = new FullCalendar.Calendar(document.querySelector("#calendar-mini"), {
    selectable: true,
    height: 350,
    locale: 'es',
    ...
    windowResize: function (arg) {
      calendar1.setOption("height", 290);
    },
    datesSet: function(info) {
      ...
      var titleText = meses[mes - 1] + ' de ' + año;
      const h3Element = document.getElementById('fc-dom-1');
      const title = h3Element.textContent;
      h3Element.innerHTML = "";
      h3Element.innerText = titleText;
    },
  });
  calendar1.render();
});
