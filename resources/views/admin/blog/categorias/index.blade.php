      ...
      <table class="table table-striped w-100 table-bordered datatable-select-inputs text-nowrap">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <button type="button" class="d-inline-flex align-items-center justify-content-center btn btn-warning rounded-circle btn-lg round-40 me-2 editButton" data-ruta="{{ route('admin.blog.categoria.edit',$categoria) }}" data-bs-toggle="modal" data-bs-target="#editModal">
                <i class="fs-5 ti ti-edit"></i>
              </button>
              <button type="button" class="d-inline-flex align-items-center justify-content-center btn btn-danger rounded-circle btn-lg round-40 deleteButton" data-ruta="{{ route('admin.blog.categoria.destroy',$categoria) }}">
...
<div class="modal fade" id="createModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="createModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
    <div class="modal-content">
      ...
      {{ html()->form('POST')->route('admin.blog.categoria.store')->attributes(['enctype' => 'multipart/form-data','style' => 'display:contents;'])->open() }}
...
<script>
  document.getElementById('btnCrear').addEventListener('click',()=>{
    fetch("{{ route('admin.blog.categoria.create') }}")
    .then(response => {
      if (!response.ok) {
