...
<body>
    <video id="player" playsinline controls data-poster="/storage/files/{{ $video->poster }}">
        <source src="/storage/files/{{ $video->video }}" type="video/mp4"/>
        ...
        <track kind="captions" label="{{ $subtitulo['label'] }}" src="{{ $subtitulo['src'] }}" srclang="{{ $subtitulo['srclang'] }}" @if ($subtitulo['default']) default @endif />
        @endforeach
    </video>
    ...
    <div class="next-video-cont d-none">
        <div class="thumbnail-video">
            ...
            <span id="nextvideoId">{{ route('reproBlog',['video'=>$siguienteVideo->idVideo]) }}</span>
        </div>
            ...
            <div class="plyr__menu__container" id="plyr-playlist" hidden="">
                <div>
                    <div id="plyr-playlist-list">
                        ...
                        <div role="menu">
                            @foreach ($videosLista as $videoL)
                            <button data-plyr="videoL" type="button" role="menuitem" class="plyr__control" aria-checked="false" value="/storage/files/{{ $videoL->video }}" data-title="{{ $videoL->titulo }}" data-poster="/storage/files/{{ $videoL->poster }}" data-captions="{{ $videoL->subtitulos }}">
                                <span>{{ $loop->iteration }}. {{ $videoL->titulo }}</span>
                            </button>
                            @endforeach
                        </div>
                    </div>
            ...
            if(!document.querySelector('button[data-plyr="playlist"]')){
                ...
                btnsListVideos.forEach(button => {
                    button.addEventListener('click',()=>{
                        ...
                        const newCaptions = button.getAttribute('data-captions');
