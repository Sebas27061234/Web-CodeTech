...
@section('title')Autores - CodeTech Admin @endsection
...
<div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-4">
  <div class="card-body px-4 py-3">
    <div class="row align-items-center">
      <div class="col-9">
        <h4 class="fw-semibold mb-8">Autores</h4>
      ...
      <table class="table table-striped w-100 table-bordered datatable-select-inputs text-nowrap">
        ...
        <tbody>
          ...
          <tr>
            <td>{{ $autor->idAutor }}</td>
            <td>{{ $autor->nombre }}</td>
            <td>{{ \Illuminate\Support\Str::limit($autor->descripcion, 35) }}</td>
            <td>
              @php
                $imagen = $autor->imagen;
                $esURL = filter_var($imagen, FILTER_VALIDATE_URL);
              @endphp

              @if ($esURL)
              <img src="{{ $imagen }}" alt="{{ $autor->nombre }}" width="100" style="border-radius: 50%">
              @else
              <img src="/storage/files/{{ $autor->imagen }}" alt="{{ $autor->nombre }}" width="100" style="border-radius: 50%">
              @endif
            </td>
            <td>
              @if ($autor->estado)
            ...
            <td>
              <button type="button" class="d-inline-flex align-items-center justify-content-center btn btn-warning rounded-circle btn-lg round-40 me-2 editButton" data-ruta="{{ route('admin.cursos.autor.edit',$autor) }}" data-bs-toggle="modal" data-bs-target="#editModal">
                <i class="fs-5 ti ti-edit"></i>
              </button>
              <button type="button" class="d-inline-flex align-items-center justify-content-center btn btn-danger rounded-circle btn-lg round-40 deleteButton" data-ruta="{{ route('admin.cursos.autor.destroy',$autor) }}">
...
<div class="modal fade" id="createModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="createModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
    <div class="modal-content">
      ...
      {{ html()->form('POST')->route('admin.cursos.autor.store')->attributes(['enctype' => 'multipart/form-data','style' => 'display:contents;'])->open() }}
...
<script>
  document.getElementById('btnCrear').addEventListener('click',()=>{
    fetch("{{ route('admin.cursos.autor.create') }}")
    .then(response => {
      if (!response.ok) {
...
@session('success')
  @if (session('success') == 'Autor creado')
  <script>
